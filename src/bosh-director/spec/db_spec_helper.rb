    ...
    def connect_database
      ...
      case ENV.fetch('DB', 'sqlite')
        when 'postgresql'
          ...
          @db_helper = Bosh::Dev::Sandbox::Postgresql.new("#{@db_name}_director", init_logger, 5432, Bosh::Core::Shell.new, user, password, host)
        when 'mysql'
          require File.expand_path('../../bosh-dev/lib/bosh/dev/sandbox/mysql', File.dirname(__FILE__))
          @db_helper = Bosh::Dev::Sandbox::Mysql.new("#{@db_name}_dns", init_logger, Bosh::Core::Shell.new, user, password, host)
        when 'sqlite'
