...
module Bosh::Dev::Sandbox
  class Postgresql
    ...
    def drop_db
      ...
      @runner.run(%Q{echo 'revoke connect on database "#{db_name}" from public; drop database "#{db_name}";' | PGPASSWORD=#{@password} psql -h #{@host} -p #{@port} -U #{@username}})
    end
    ...
    def truncate_db
      ...
      cmds_cmd = %Q{PGPASSWORD=#{@password} psql -h #{@host} -p #{@port} -U #{@username} #{db_name} -c "
